...
import { getAuth, signInWithEmailAndPassword, signInWithCustomToken, onAuthStateChanged, GoogleAuthProvider, signInWithPopup, signInAnonymously, signOut } from 'firebase/auth';

// Import components from the 'components' subdirectory
import Preloader from './components/Preloader';
import LoginModal from './components/LoginModal';
import CreatorCard from './components/CreatorCard';
import CreatorProfile from './components/CreatorProfile';

// Define the Creator interface directly within this file, or import it from `./components/types`
interface Creator {
  id: number;
  name: string;
  rating: number;
...
const creators: Creator[] = [
  ...
  },
  {
    ...
    price: 31.50,
    isAd: true,
    image: 'https://placehold.co/400x400/000000/FFFFFF?text=PORTRAIT',
    type: 'Portrait',
  },
  {
    id: 9,
  ...
  },
  {
    ...
    price: 34.00,
    isAd: false,
    image: 'https://placehold.co/400x400/000000/FFFFFF?text=STREET',
    type: 'Street',
  },
  {
    id: 11,
...
const Home = () => {
  ...
  // Conditional rendering for the main content or the profile page
  if (selectedCreator) {
    ...
    <div className="bg-gray-950 text-white min-h-screen font-sans">
      ...
      <header className="relative z-50 py-4 px-6 md:px-12 flex justify-between items-center">
        <div className="h-10 w-auto">
          <img src="/logo.png" alt="Naughty Den Logo" className="h-10 w-auto" />
        </div>
          ...
          {user ? (
            <>
              <div className="flex items-center space-x-2">
                {user.photoURL && (
                  <img src={user.photoURL} alt="Profile" className="w-8 h-8 rounded-full border border-pink-600" />
                )}
      ...
      <section className="relative h-[80vh] flex items-center justify-center text-center">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1648064139778-7ab990300d6e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')" }}>
